...
5. **If in MAIN SESSION**: Check `/root/.openclaw/workspace/quanyin_pending.txt` — if it exists and was generated today, prepend its content as a sent message to the user (don't reply to it, just queue it as a proactive push via `message` tool), then delete it.
